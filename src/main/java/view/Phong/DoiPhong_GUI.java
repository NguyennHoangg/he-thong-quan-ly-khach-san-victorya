...
public class DoiPhong_GUI extends BorderPane {
        ...
        private Label lblSoPhongChenhLechGiaTri, lblLoaiPhongChenhLechGiaTri, lblTongTienChenhLechGiaTri;
        ...
        public DoiPhong_GUI() {
                ...
                containPhai.setPadding(new Insets(20, 10, 5, 45));
                containPhai.getChildren().addAll(
                                taoPhongBanDau(), taoPhongSau(), taoPhiChenhLech(), taoNutXacNhan());

                HBox root = new HBox(containTrai, containPhai);
                root.getStylesheets().add(getClass().getResource("/css/Button.css").toExternalForm());
                root.getStylesheets().add(getClass().getResource("/css/Label.css").toExternalForm());
                this.setCenter(root);
        ...
        private void lamMoiGUI() {
                ...
                lblSoPhongChenhLechGiaTri.setText("-");
                lblLoaiPhongChenhLechGiaTri.setText("-");
                lblTongTienChenhLechGiaTri.setText("-");

                // 5. Reset list phòng đã chọn
        ...
        private VBox taoPhongBanDau() {
                VBox box = taoKhungThongTin("#2CD9FF", "Phòng ban đầu");
        ...
        private VBox taoPhiChenhLech() {
                VBox box = taoKhungThongTin("#EC221F", "Phí chênh lệch");

                lblSoPhongChenhLechGiaTri = taoGiaTri();
                lblLoaiPhongChenhLechGiaTri = taoGiaTri();
                lblTongTienChenhLechGiaTri = taoGiaTriBold();

                box.getChildren().addAll(
                                taoRow("Số phòng:", lblSoPhongChenhLechGiaTri),
                                taoRow("Loại phòng:", lblLoaiPhongChenhLechGiaTri),
                                taoRow("Đơn vị tính:", new Label("vnđ/h")),
                                taoRow("Giá chênh lệch:", lblTongTienChenhLechGiaTri));

                return box;
        }

        private void capNhatChenhLech() {
                if (ctpdpChonDoi == null || phongDaChon == null)
                        return;

                lblSoPhongChenhLechGiaTri
                                .setText(ctpdpChonDoi.getPhong().getSoPhong() + " → " + phongDaChon.getSoPhong());
                lblLoaiPhongChenhLechGiaTri.setText(ctpdpChonDoi.getPhong().getLoaiPhong().getTenLoaiPhong()
                                + " → " + phongDaChon.getLoaiPhong().getTenLoaiPhong());

                double tienOld = ctpdpChonDoi.getPhong().getLoaiPhong().getGia();
                double tienNew = phongDaChon.getLoaiPhong().getGia();
                double chenh = tienNew - tienOld;

                lblTongTienChenhLechGiaTri.setText(String.format("%,.0f VND", chenh));
        }
