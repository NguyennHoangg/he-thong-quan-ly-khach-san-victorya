    ...
    public class QuanLiHoaDon_GUI extends BorderPane {
        ...
        private Node xayDungKhuVucNoiDung() {
            ...
            // thanh bộ lọc: thu nhỏ pill & padding
        ...
        // Dialog chi tiết: Thông tin + Bảng Chi tiết + Bảng Dịch vụ
        ...
        private void khoiTaoSuKien() {
            ...
            btnTaiLai.setOnAction(new EventHandler<ActionEvent>() {
                @Override public void handle(ActionEvent e) {
                    resetBoLoc();   // <-- XÓA TOÀN BỘ BỘ LỌC
                    taiDuLieu();    // nạp lại dữ liệu gốc
                }
            });
        }
        ...
        /** Đưa bảng về trạng thái ban đầu (không lọc, không sort, controls rỗng) */
        private void resetBoLoc() {
            ...
            bang.getSortOrder().clear();  // xoá sắp xếp cột hiện tại
            duLieuLoc.setPredicate(hd -> true); // clear predicate tạm thời
        }
        ...
        private static Node pill(Node inner) {
            ...
            box.setAlignment(Pos.CENTER_LEFT);
            box.setPadding(new Insets(5, 10, 5, 10)); // pill nhỏ hơn
        ...
        private static TrangThaiHD mapTrangThai(String raw) {
            ...
            if (x.contains("đã") || x.contains("da") || x.contains("hoàn") || x.contains("hoan")) return TrangThaiHD.HOAN_THANH;
            if (x.contains("hủy") || x.contains("huy")) return TrangThaiHD.DA_HUY;
            if (x.contains("chờ") || x.contains("cho")) return TrangThaiHD.DANG_CHO;
        ...
        public enum TrangThaiHD {
            ...
            public static StringConverter<TrangThaiHD> converter() {
                return new StringConverter<TrangThaiHD>() {
                    @Override public String toString(TrangThaiHD s) { return s == null ? "" : s.nhan; }
                    @Override public TrangThaiHD fromString(String s) {
